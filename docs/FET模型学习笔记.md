...
## 9.6 迭代重打标突破（2026-06-17，关键学习点）

**进展链（val mid，90/10）**：
| 阶段 | val mid | 杠杆 |
|---|---:|---|
| FET baseline | 0.9184 | 架构 |
| +ELR 6ep | 0.9214 | 模型级正则(去噪) |
| +ELR 8ep | 0.9237 | 同上+更长 → **触及 LoRA 0.9244 同档天花板** |
| **+迭代重打标 round1** | **0.9289** | **数据级去噪 → 突破 soup 0.9267！** |

**机制（学习要点）**：
1. `prepare_targets` 原用**冻结特征上的线性头**当老师选伪标签——很弱，只回收 84 个被丢样本。
2. 换成**训好的 FET+ELR 整图模型**（mean_conf 0.948）当老师 → **回收 1634 个（19×）**。强模型是远好的"标签清洗器"。
3. 用更干净的标签重训 → **val 0.9289**，**首次单模型超过 soup**。
4. **核心教训**：**模型级正则(ELR)只能把单模型推到架构天花板(~LoRA 0.924)；要突破，得从数据层面清噪(迭代重打标)。** 这印证"瓶颈是噪声不是架构"。
5. **防确认偏差**：只在"模型预测 ∩ kNN多数票 共识"且高置信时才回收/改标（kNN 来自冻结特征，是相对独立信号）。
6. **与长训练负结果的区别**：60轮长训练 noisy_all 虚高但榜降（**记忆错标**）；迭代重打标是**向共识纠错**（标签变干净），机制相反，应利于干净测试——但仍需榜分确认。

**复利路线**：round2(teacher=iter1，更强) → round3… 每轮更强的老师清更多噪。配合 512/FET汤。
