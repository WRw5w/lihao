#!/usr/bin/env bash
# Resume auto-opt queue: remaining candidates only.
# c448_drecall already DONE (mid_03_06=0.9184) and pushed to auto-opt/c448_drecall.
set -u
...
run_candidate() {
  ...
  rm -rf "${wd}"; mkdir -p "${wd}/lora"   # clean any partial output from the interrupted run
...
PYEOF
  ...
  mid=${best%%|*}
  {
    ...
    echo "- config: \`${extra}\`"
    echo "- best: ${best}"
    echo "- baseline to beat: c448_drecall mid_03_06=0.9184 (> C448 0.9091)"
  } > "${wd}/RESULT.md"
...
echo "[auto-opt] RESUME QUEUE COMPLETE"
